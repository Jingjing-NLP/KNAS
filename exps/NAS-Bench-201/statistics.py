...
from lib.nas_201_api  import ArchResults, ResultsCount
...
def create_result_count(used_seed, dataset, arch_config, results, dataloader_dict):
  xresult     = ResultsCount(dataset,{},  results['train_acc1es'], results['train_losses'], \
                                0, results['flop'], arch_config, used_seed, 1, None)
  print('train_times' in results)
  ...
  if 'train_times' in results: # new version
    ...
    print(results['train_acc1es'], results['train_losses'], results['train_times'])
...
def account_one_arch(arch_index, arch_str, checkpoints, datasets, dataloader_dict):
  ...
  for checkpoint_path in checkpoints:
    ...
    for dataset in datasets:
      ...
      #print('xresult',xresult, information)
  return information
...
def simplify(save_dir, meta_file, basestr, target_dir):
  ...
  print(subdir2archs, save_dir / target_dir)
  assert (save_dir / target_dir) in subdir2archs, 'can not find {:}'.format(target_dir)
  arch2infos, datasets = {}, ( 'cifar10', 'cifar100', 'ImageNet16-120')
  ...
  for idx, arch_index in enumerate(arch_indexes):
    ...
    #try:
    #print('checkpoints',arch_index, checkpoints)
    arch_info_full = account_one_arch(arch_index, meta_archs[int(arch_index)], checkpoints, datasets, dataloader_dict)
    arch_info_less = arch_info_full#account_one_arch(arch_index, meta_archs[int(arch_index)], ckps_less, ['cifar10-valid'], dataloader_dict)
    #print(arch_info_full)
    num_seeds[ len(checkpoints) ] += 1
    #except:/account_one_arch
  ...
  # measure time
  print(arch2infos, arch_indexes,evaluated_indexes)
