...
def train_and_eval(arch, nas_bench, extra_info, dataname='cifar10-valid', use_012_epoch_training=True):
  ...
  if use_012_epoch_training and nas_bench is not None:
    ...
    #valid_acc = 0
    #if 777 in nas_bench.query_by_index(arch_index, dataname):#print(arch_index,dataname,nas_bench.query_by_index(arch_index, dataname))
    #  valid_acc = nas_bench.query_by_index(arch_index, dataname)[777].get_eval('ori-test')['accuracy']
...
def regularized_evolution(cycles, population_size, sample_size, time_budget, random_arch, mutate_arch, nas_bench, extra_info, dataname):
  ...
  # another.
  while len(history) < 100-population_size:
...
if __name__ == '__main__':
  ...
  if args.rand_seed < 0:
    ...
    for i in range(3):
      ...
      if 777 in nas_bench.query_by_index(index, args.dataset):
          acc  = nas_bench.query_by_index(index, args.dataset)[777].get_eval('ori-test')['accuracy']
      else:
          acc  = nas_bench.query_by_index(index, args.dataset)[888].get_eval('ori-test')['accuracy']
          #print(acc)
      avg += acc
    print(avg/3)
